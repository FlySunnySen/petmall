...
class GoodsLogic extends Model {
	...
	public function getSpecItem($spec_id) {
		...
		$arr = self::get_id_val($arr, 'id', 'item');
		return $arr;
	}
	...
	public function getSpecInput($goods_id, $spec_arr) {
		...
		foreach ($spec_arr2 as $k => $v) {
			...
			if (!array_key_exists($item_key, $keySpecGoodsPrice)) {
				...
				$keySpecGoodsPrice[$item_key]['market_price'] = 0; //市场价默认为0
				$keySpecGoodsPrice[$item_key]['cost_price'] = 0; //成本价默认为0
				$keySpecGoodsPrice[$item_key]['commission'] = 0; //佣金默认为0
				$keySpecGoodsPrice[$item_key]['sku'] = 0; //佣金默认为0
			}
	...
	function getSortBrands($cat_id = 0) {
		...
		$brandList = model("Brand")->cache(true)->where($brand_where)->select();
		$brandIdArr = model("Brand")->cache(true)->where($brand_where)->where("name in (select `name` from `" . C('database.prefix') . "brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');

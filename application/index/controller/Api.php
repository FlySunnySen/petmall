...
class Api extends Base {
	...
	*/
	...
	public function issetMobileOrEmail() {
		$mobile = input("mobile", '0');
		$users = Db::name('users')->where("email", $mobile)->whereOr('mobile', $mobile)->find();
		if ($users) {
			exit('1');
		} else {
			exit('0');
	...
	public function queryExpress() {
		...
		if ($express_switch == 1) {
			...
			$kdniao = new \kdniao();
			$data['OrderCode'] = empty(input('order_sn')) ? date('YmdHis') : input('order_sn');
			$data['ShipperCode'] = input('shipping_code');

...
class CartLogic extends Model {
	...
	private function addPromGoodsCart() {
		...
		if ($userWantGoodsNum > $storeCount) {
			//用户购买量不得超过库存
...
//            $carts = $Cart->where(['combination_group_id' => $cart['combination_group_id'], 'id' => ['neq', $cart['id']]])->select();
	 ...
	 * @return array
	 */
	public function AsyncUpdateCart($cart = [], $user_id) {
		...
		$cartWhere['user_Uid'] = $_SESSION['uid'];
		if (!empty($cartNoSelectedId)) {
			Db::name('cart')->where('id', 'IN', $cartNoSelectedId)->where($cartWhere)->update(['selected' => 0]);
		}
		if (empty($cartSelectedId)) {
			$cartPriceInfo = self::getCartPriceInfo();
			$cartPriceInfo['cartList'] = $cartList;
			return ['status' => 1, 'msg' => '购物车没选中商品', 'result' => $cartPriceInfo];
		}
		$cartList = Db::name('cart')->where('id', 'IN', $cartSelectedId)->where($cartWhere)->select();
		$cartListArr = $cartList;
		//查出搭配购的商品
		foreach ($cartListArr as $cartKey => $cartVal) {
			if ($cartListArr[$cartKey]['selected'] == 0) {
				Db::name('cart')->where('id', 'IN', $cartSelectedId)->where($cartWhere)->update(['selected' => 1]);
				break;
			}
		}

		if ($cartListArr) {
			foreach ($cartListArr as $key => $val) {
				# code...
				if ($val['item_id'] == 0) {
					$cartListArr[$key]['price'] = Db::name('good')->where('id', '=', $val['id'])->value('goods_price');
				} else {
					$cartListArr[$key]['price'] = Db::name('spec_goods_price')->where('item_id', '=', $val['item_id'])->value('price');

				}
				$cartListArr[$key]['cut_fee'] = $cartListArr[$key]['price'] * $val['num'];
			}
			$cartPriceInfo = self::getCartPriceInfo($cartListArr);
	...
	public function getCartPriceInfo($cartList = null) {
		...
		if ($cartList) {
			foreach ($cartList as $cartKey => $cartItem) {
				$goods_num += $cartItem['num'];
				$total_fee += $cartItem['cut_fee'];
			}
		}
		return compact('goods_num', 'total_fee');
	}
	...
	public function getCouponCartList($cartList, $userCouponList) {
		...
		foreach ($userCouponArray as $couponKey => $couponItem) {
			if ($userCouponArray[$couponKey]['coupon']['use_type'] == 0) {
				//全店使用优惠券
				if ($cartList['total_fee'] >= $userCouponArray[$couponKey]['coupon']['condition']) {
					//订单商品总价是否符合优惠券购买价格

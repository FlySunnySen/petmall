...
class GoodsLogic extends Model {
	...
	public function get_filter_brand($goods_id_arr, $filter_param, $action) {
		...
		$map['brand_id'] = ['>', 0];
		$brand_id_arr = model('goods')->where($map)->column('brand_id');
		$list_brand = model('brand')->where(['id' => ['in', $brand_id_arr], 'is_hot' => 1])->limit('30')->select();
	...
	public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0) {
		...
		$spec = model('spec')->where(array('search_index' => 1))->getField('id,name');
		$spec_item = model('spec_item')->where(array('spec_id' => array('in', array_keys($spec))))->getField('id,spec_id,item');
	...
	public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0) {
		$goods_id_str = implode(',', $goods_id_arr);
		$goods_id_str = $goods_id_str ? $goods_id_str : '0';
		$goods_attr = model('goods_attr')->where(['goods_id' => ['in', $goods_id_str], 'attr_value' => ['<>', '']])->select();
		// $goods_attr = model('goods_attr')->where("attr_value != ''")->select();
		$goods_attribute = model('goods_attribute')->where("attr_index = 1")->getField('attr_id,attr_name,attr_index');
		if (empty($goods_attr)) {
			if ($mode == 1) {
				return array();
			}
		...
		foreach ($goods_attr as $k => $v) {
			...
			$v['attr_value'] = trimodel($v['attr_value']);
	...
	public function commentStatistics($goods_id) {
		...
		$c1 = model('comment')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (4,5)')->count();
		$c2 = model('comment')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (3)')->count();
		$c3 = model('comment')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (0,1,2)')->count();
		$c4 = model('comment')->where($commentWhere)->where("img !='' and img NOT LIKE 'N;%'")->count(); // 晒图
		$c0 = $c1 + $c2 + $c3; // 所有评论
		if ($c0 <= 0) {
			$rate1 = 100;
			$rate2 = 0;
	...
	public function get_spec($goods_id) {
		...
		$keys = model('SpecGoodsPrice')->where("goods_id", $goods_id)->column("GROUP_CONCAT(`key` ORDER BY store_count desc SEPARATOR '_') ");
		$filter_spec = array();
		if ($keys) {
	...
	public function get_siblings_cate($cat_id) {
		...
		$cate_info = model('goods_category')->where("id", $cat_id)->find();
		$siblings_cate = model('goods_category')->where(['id' => ['<>', $cat_id], 'parent_id' => $cate_info['parent_id']])->select();
	...
	public function get_look_see($goods) {
		return model('goods')->where(['goods_id' => ['<>', $goods['goods_id']], 'cat_id' => $goods['cat_id'], 'is_on_sale' => 1])->limit(12)->select();
	}
	...
	function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5) {
		...
		$goods_id_str = implode(',', $goods_id_arr);
		$goods_id_str = $goods_id_str ? $goods_id_str : '0';
		$priceList = model('goods')->where("goods_id", "in", $goods_id_str)->getField('shop_price', true); //where("goods_id in($goods_id_str)")->select();
		rsort($priceList);
	...
	function get_goods_cate(&$goodsCate) {
		...
		if ($goodsCate['level'] == 1) {
			...
			$parent = model('GoodsCategory')->where("id", $goodsCate['parent_id'])->order('`sort_order` desc')->find(); //父类
	...
	function getGoodsIdByBrandPrice($brand_id, $price) {
		...
		if ($brand_id) // 品牌查询
		{
			...
			$brand_select_goods = model('goods')->whereIn('brand_id', $brand_id_arr, 'or')->getField('goods_id', true);
		}
		if ($price) // 价格查询
		{
			$price = explode('-', $price);
			...
			$price_select_goods = model('goods')->where($price_where)->getField('goods_id', true);
		}
	...
	function getRegionList() {
		...
		$parent_region = model('region')->field('id,name')->where(array('level' => 1))->cache(true)->select();
		$ip_location = array();
		$city_location = array();
		foreach ($parent_region as $key => $val) {
			$c = model('region')->field('id,name')->where(array('parent_id' => $parent_region[$key]['id']))->order('id asc')->cache(true)->select();
	...
	function getParentRegionList($cid) {
		//$pids = '';
		$pids = array();
		$parent_id = model('region')->where(array('id' => $cid))->getField('parent_id');
		if ($parent_id != 0) {
			//$pids .= $parent_id;
	...
	public function getFreight($goodsArr, $region_id) {
		...
		foreach ($template_list as $templateVal => $goodsArr) {
			...
			$freightLogic->setGoodsNumodel($temp['goods_num']);
	...
	public function add_visit_log($user_id, $goods) {
		$record = model('goods_visit')->where(array('user_id' => $user_id, 'goods_id' => $goods['goods_id']))->find();
		if ($record) {
			model('goods_visit')->where(array('user_id' => $user_id, 'goods_id' => $goods['goods_id']))->save(array('visittime' => time()));
		} else {
			$visit = array('user_id' => $user_id, 'goods_id' => $goods['goods_id'], 'visittime' => time(), 'cat_id' => $goods['cat_id'], 'extend_cat_id' => $goods['extend_cat_id']);
			model('goods_visit')->add($visit);
		}
	}
	...
	public function isCollectGoods($user_id, $goods_id) {
		$collect = model('goods_collect')->where(['user_id' => $user_id, 'goods_id' => $goods_id])->find();
		return $collect ? 1 : 0;
	}
	/**
	 * 获取促销商品数据
...
//        $goods_where['goods_name'] = array("exp", " NOT REGEXP '华为|荣耀|小米|合约机|三星|魅族' ");//临时屏蔽,苹果APP审核过了之后注释
	...
	public function getHomeAdv() {
		...
		$adv = model("ad")->field(array('ad_link', 'ad_name', 'ad_code'))
	...
	public function getAppHomeAdv($isBanner = true) {
		...
		$adv = model("ad")->field(array('ad_link', 'ad_name', 'ad_code', 'media_type,pid'))
	...
	public function getFlashSaleGoods($count, $page = 1, $start_time = 0, $end_time = 0) {
		...
		$flash_sale_goods = model('flash_sale')->alias('f')
	...
	public function getSimilar($goods_id, $p, $count) {
		$goods = model('goods')->field('cat_id')->where('goods_id', $goods_id)->find();
		if (empty($goods)) {
			return [];
		}

		$where = ['goods_id' => ['<>', $goods_id], 'cat_id' => $goods['cat_id']];
		$goods_list = model('goods')->field("goods_id,goods_name,shop_price,is_virtual")
	...
	public function integralMall($rank, $user_id, $p = 1) {
		...
		if ($rank == 'exchange' && !empty($user_id)) {
			//获取用户积分
			$user_pay_points = intval(model('users')->where(array('user_id' => $user_id))->getField('pay_points'));
		...
		$goods_list_count = model('goods')->where($goods_where)->count(); //总数
		$goods_list = model('goods')->field('goods_id,goods_name,shop_price,market_price,exchange_integral,is_virtual')
	...
	function getSortBrands() {
		$brandList = model("Brand")->select();
		$brandIdArr = model("Brand")->where("name in (select `name` from `" . C('database.prefix') . "brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
		$goodsCategoryArr = model('goodsCategory')->where("level = 1")->getField('id,name');
		$nameList = array();
		foreach ($brandList as $k => $v) {

			$name = getFirstCharter($v['name']) . '  --   ' . $v['name']; // 前面加上拼音首字母

			if (array_key_exists($v[id], $brandIdArr) && $v['cat_id']) {
				// 如果有双重品牌的 则加上分类名称
	...
	public function getOrderPromSimpleInfo($goods) {
		...
		$data = [];
		$po = model('prom_order')->where(['start_time' => ['<=', $cur_time], 'end_time' => ['>', $cur_time], 'is_close' => 0])->select();
		if (!empty($po)) {
			foreach ($po as $p) {
	...
	function getSortCategory() {
		$categoryList = model("GoodsCategory")->getField('id,name,parent_id,level');
		$nameList = array();

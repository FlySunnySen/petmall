...
class GoodsLogic extends Model {
	...
	public function get_filter_brand($goods_id_arr, $filter_param, $action) {
		...
		$map['id'] = ['in', $goods_id_arr];
		$map['brand_id'] = ['>', 0];
		$brand_id_arr = Db::name('good')->where($map)->column('brand_id');
		$list_brand = Db::name('brand')->where(['id' => ['in', $brand_id_arr]])->limit('30')->select();

		foreach ($list_brand as $k => $v) {
			// 帅选参数
			$filter_param['brand_id'] = $v['id'];
			$list_brand[$k]['href'] = urldecode(url("Goods/$action", $filter_param, ''));
		}
		return $list_brand;
	}
	...
	public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0) {
		...
		$spec_key = DB::query("select group_concat(`key` separator  '_') as `key` from pet_spec_goods_price where goods_id in($goods_id_str)"); //where("goods_id in($goods_id_str)")->select();
		...
		$spec = Db::name('spec')->where(array('search_index' => 1))->column('id,name');
		// var_dump($spec);die;
		$spec_item = Db::name('spec_item')->where(array('spec_id' => array('in', array_keys($spec))))->column('id,spec_id,item');
		$list_spec = array();
		...
		foreach ($spec_key as $k => $v) {
			...
			$list_spec[$spec_item[$v]['spec_id']]['item'][] = array('key' => $spec_item[$v]['spec_id'], 'val' => $v, 'item' => $spec_item[$v]['item'], 'href' => urldecode(url("Goods/$action", $filter_param, '')));
		}
	...
	public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0) {
		...
		$goods_attribute = Db::name('goods_attribute')->where("attr_index = 1")->getField('attr_id,attr_name,attr_index');
		if (empty($goods_attr)) {
			if ($mode == 1) {
				return array();
			}
		...
		foreach ($goods_attr as $k => $v) {
			...
			$v['attr_value'] = triDb::name($v['attr_value']);
			...
			$list_attr[$v['attr_id']]['attr_value'][] = array('key' => $v['attr_id'], 'val' => $v['attr_value'], 'attr_value' => $v['attr_value'], 'href' => url("Goods/$action", $filter_param, ''));
	...
	public function commentStatistics($goods_id) {
		$commentWhere = ['is_show' => 1, 'goods_id' => $goods_id, 'pid' => 0, 'user_id' => ['gt', 0]];
	...
	public function get_siblings_cate($cat_id) {
		...
		$siblings_cate = Db::name('goods_category')->where(['id' => ['<>', $cat_id], 'pid' => $cate_info['pid']])->select();
	...
	function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5) {
		...
		$goods_id_str = implode(',', $goods_id_arr);
		$goods_id_str = $goods_id_str ? $goods_id_str : '0';
		$priceList = Db::name("good")->where("id", "in", $goods_id_str)->column('goods_price', true); //where("goods_id in($goods_id_str)")->select();
		rsort($priceList);
		...
		for ($i = 0; $i < $c; $i++) {
			...
			if ($i == 0) {
				$parr[] = array('value' => "{$end}元以下", 'href' => urldecode(url("Goods/$action", $filter_param, '')));
			} elseif ($i == ($c - 1) && ($max_price > $end)) {
				$parr[] = array('value' => "{$end}元以上", 'href' => urldecode(url("Goods/$action", $filter_param, '')));
			} else {
				$parr[] = array('value' => "{$start}-{$end}元", 'href' => urldecode(url("Goods/$action", $filter_param, '')));
			}
	...
	function get_filter_menu($filter_param, $action) {
		...
		// 品牌
		if (!empty($filter_param['brand_id'])) {
			$brand_list = Db::name('brand')->column('id,brand_name');
		...
		if (!empty($filter_param['attr'])) {
			$goods_attribute = Db::name('goods_attribute')->column('attr_id,attr_name');
			...
			foreach ($attr_group as $k => $v) {
				...
				$attr_menu['href'] = urldecode(url("Goods/$action", $tmp2, ''));
				$menu_list[] = $attr_menu;
			}
		}
		// 价格
		if (!empty($filter_param['price'])) {
			...
			$price_menu['href'] = urldecode(url("Goods/$action", $filter_param, ''));
	...
	function get_goods_cate(&$goodsCate) {
		...
		$cateAll = self::get_goods_category_tree();
		if ($goodsCate['level'] == 1) {
			$cateArr = $cateAll[$goodsCate['id']]['tmenu'];
			$goodsCate['parent_name'] = $goodsCate['type_name'];
			$goodsCate['select_id'] = 0;
		} elseif ($goodsCate['level'] == 2) {
			$cateArr = $cateAll[$goodsCate['pid']]['tmenu'];
			$goodsCate['parent_name'] = $cateAll[$goodsCate['pid']]['type_name']; //顶级分类名称
			$goodsCate['open_id'] = $goodsCate['id']; //默认展开分类
			$goodsCate['select_id'] = 0;
		} else {
			$parent = Db::name('good_type')->where("id", $goodsCate['pid'])->order('type_order')->find(); //父类
			$cateArr = $cateAll[$parent['pid']]['tmenu'];
			$goodsCate['parent_name'] = $cateAll[$parent['pid']]['type_name']; //顶级分类名称
	...
	function getGoodsIdByBrandPrice($brand_id, $price) {
		...
		if ($brand_id) // 品牌查询
		{
			...
			$brand_select_goods = Db::name('goods')->whereIn('brand_id', $brand_id_arr, 'or')->getField('goods_id', true);
		}
		if ($price) // 价格查询
		{
			$price = explode('-', $price);
			...
			$price_select_goods = Db::name('goods')->where($price_where)->getField('goods_id', true);
		}
	...
	function get_goods_category_tree() {
		...
		$cat_list = Db::name('good_type')->where(['is_show' => 1])->order('type_order')->select(); //所有分类
		foreach ($cat_list as $k => $v) {
			/* 给分类附上level */
			$cat_list[$k]['level'] = count(explode('_', $v['parent_id_path'])) - 1;
		}
		if ($cat_list) {
	...
	function getRegionList() {
		...
		foreach ($parent_region as $key => $val) {
			$c = Db::name('region')->field('id,name')->where(array('pid' => $parent_region[$key]['id']))->order('id asc')->cache(true)->select();
	...
	function getParentRegionList($cid) {
		//$pids = '';
		$pids = array();
		$pid = Db::name('region')->where(array('id' => $cid))->getField('pid');
		if ($pid != 0) {
			//$pids .= $pid;
			array_push($pids, $pid);
			$npids = $this->getParentRegionList($pid);
			if (!empty($npids)) {
				//$pids .= ','.$npids;
...
//        $goods_where['goods_name'] = array("exp", " NOT REGEXP '华为|荣耀|小米|合约机|三星|魅族' ");//临时屏蔽,苹果APP审核过了之后注释
	...
	public function getFlashSaleGoods($count, $page = 1, $start_time = 0, $end_time = 0) {
		...
		$flash_sale_goods = Db::name('flash_sale')->alias('f')
	...
	public function integralMall($rank, $user_id, $p = 1) {
		...
		if ($rank == 'exchange' && !empty($user_id)) {
			//获取用户积分
			$user_pay_points = intval(Db::name('users')->where(array('user_id' => $user_id))->getField('pay_points'));
		...
		$goods_list = Db::name('goods')->field('goods_id,goods_name,shop_price,market_price,exchange_integral,is_virtual')
	...
	function getSortBrands() {
		...
		$brandIdArr = Db::name("Brand")->where("name in (select `name` from `" . C('database.prefix') . "brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
	...
	public function getOrderPromSimpleInfo($goods) {
		...
		$data = [];
		$po = Db::name('prom_order')->where(['start_time' => ['<=', $cur_time], 'end_time' => ['>', $cur_time], 'is_close' => 0])->select();
		if (!empty($po)) {
			foreach ($po as $p) {
	...
	function getSortCategory() {
		$categoryList = Db::name("GoodsCategory")->getField('id,name,pid,level');
		$nameList = array();
		foreach ($categoryList as $k => $v) {
			...
			/*
	            ...
	            $pid = $v['pid'];
	            if($pid)
	                $name .= '--'.$categoryList[$pid]['name'];
	            // 找他 爷爷
	            $pid = $categoryList[$v['pid']]['pid'];
	            if($pid)
	                $name .= '--'.$categoryList[$pid]['name'];
*/
			$nameList[] = $v['name'] = $name;
